...
**Marqueur** — deux catégories laissent l'entrée au registre en la marquant : `aggravee` et
`inverifiable`. Forme du marqueur, place exacte dans l'entrée et raison pour laquelle l'intitulé
n'est jamais touché : `../../implementation-tracker/references/dette.md`, « Marqueur d'une entrée
relue ». Ce fichier-ci dit **quelles** catégories marquent, `dette.md` dit **comment** — la tenue du
registre lui appartient.
...
catégories ne le dit, et aucune n'a à le dire : la catégorie juge le **fait**, pas sa rédaction. Le
bloc le signale, l'utilisateur arbitre, le texte se corrige sans que le verdict change —
...
**Destination** — retirée du registre, déplacée en **fin** de `technical-debt-solde.md`, complétée
par `**Soldé le <date> par <ce qui l'a soldé>**` et la commande.
...
**Destination** — `technical-debt-ecarte.md`, motif `non pertinent`.
...
refait ensuite, et deux chantiers
successifs peuvent verser le même constat sous deux formulations.

**Preuve exigée** — aucune commande : **l'intitulé exact de l'entrée conservée**, cité dans le
verdict. Sans lui, le bloc n'est pas arbitrable.

**Destination** — `technical-debt-ecarte.md`, motif `doublon`, en nommant l'entrée conservée. La
conservée est toujours **la plus ancienne** : c'est elle qui porte la date d'origine du constat.
...
**Destination** — `technical-debt-ecarte.md`, motif `pas une dette`.
...
pas de l'origine. Marqueur `(aggravée) <date>` sous le titre, intitulé inchangé.
...
**Destination** — inchangée. Aucune écriture, aucun marqueur.
...
**Preuve exigée** — aucune. Le bloc dit **pourquoi** rien n'est exécutable ; c'est ce qui le
distingue d'un classement paresseux.

**Destination** — reste au registre, marqueur `(invérifiable en revue) <date>` sous le titre. Les
revues suivantes la lisent sans la réinstruire, sauf si son **Pour solder** est devenu actionnable.
