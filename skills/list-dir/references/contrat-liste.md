...
que pour `reseed` : [Provenance et péremption](#provenance-et-péremption).
...
la définition a bougé depuis, et `reseed` rattrape **sur ordre** —
[Provenance et péremption](#provenance-et-péremption). L'autorité, elle, n'a pas bougé d'un pouce :
c'est toujours la copie qui s'applique, et rien ne l'écrase sans qu'on l'ait demandé.
...
Une liste semée porte une copie du contrat, et cette copie est la seule règle appliquée. Une skill
qui écrit dans cette liste suppose pourtant qu'elle vaut sa semence — et le jour où la définition
évolue, rien ne le disait. La table `[origin]` comble ce trou sans rendre à la définition la moindre
autorité.
...
> *Mode de défaillance* — une estampille écrite par `init` d'après ce qu'on lui a tapé nommerait la
> définition **visée**, pas celle qui a réellement semé. `--def <nom>` sur une semence qui déclare
> un autre nom est refusé en code 2 pour la même raison.
...
> *Mode de défaillance* — sans la semence gardée, il n'y a pas de troisième point : une clé qui
> diffère ne dit plus lequel des deux côtés a bougé, et la moindre reformulation de description
> dans la définition se lirait comme un conflit.
...
conformes : `migrate` les remet en ligne, et c'est un second geste — [Quand le contrat
change](#quand-le-contrat-change).
