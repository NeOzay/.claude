...
def test_nom_de_patron_refuse_en_disant_ce_qu_il_est(tmp_path: Path) -> None:
    """Un patron n'est pas une définition : il projette une liste qui existe déjà.
    ...
    assert "--patron review" in r.message
...
def test_un_nom_composite_mal_forme_ne_conseille_pas_une_commande_inutilisable() -> None:
    ...
    « technical-debt/ » nommerait un patron vide et conseillerait « --patron  » ;
    « a/b/c » inventerait un patron « b/c ». Un message qui envoie taper une commande
    vouée à échouer est un échec ouvert de plus, pas une aide.
    """
    for nom in ("technical-debt/", "/x", "../x", "a/b/c"):
        r = resolve(nom, [])
        assert not r, nom
        assert "--patron " not in r.message, nom
        assert "n'est pas un nom de semence" in r.message, nom


def test_le_refus_du_patron_ne_depend_d_aucune_racine() -> None:
    """Il se prononce sur la FORME du nom : aucune racine n'a besoin d'exister."""
    r = resolve("a/b", [])

    assert not r
    assert "patron" in r.message
